"""
Chains management page for RAI WebUI.
"""
...
@ui.page('/chains')
@layout
def chains_page() -> None: # noqa: PLR0915
    # pylint: disable=too-many-statements
    """Render the chains page."""
                ...
                with ui.card().classes('w-full q-mb-sm'):
                    with ui.row().classes('items-center justify-between w-full'):
                        ...
                        with ui.row():
                            ui.button(
                                icon='edit',
                                on_click=lambda c=chain: open_editor(c)
                            ).props('flat round dense')
                            ui.button(
                                icon='delete',
                                color='negative',
                                on_click=lambda c=chain: delete_chain(c)
                            ).props('flat round dense')

    def delete_chain(chain: Dict[str, Any]) -> None:
    ...
    def open_editor(chain: Optional[Dict[str, Any]] = None) -> None:
        editor_dialog.clear()
                ...
                with steps_container:
                    for i, step in enumerate(current_steps):
                        with ui.row().classes('items-center w-full'):
                            ...
                            def update_step(val: str, idx: int = i) -> None:
                            ...
                            ui.button(
                                icon='delete',
                                color='negative',
                                on_click=lambda idx=i: remove_step(idx)
                            ).props('flat round dense')
